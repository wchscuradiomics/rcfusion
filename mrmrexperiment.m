...
nomfsmethods = {'mrmr','lasso','univaranalysis|0.1','univaranalysis|0.01','univaranalysis|0.05'};
...
prelexptparams = ctable2struct(meshvarargin2grid(csvs,difsigtstthresholds,corrcoefthresholds,alphas,...
  nlims,nomfsmethods)); % criterias

xlsxname = 'mrmr_prelexptresults.xlsx';
...
parfor i=1:height(prelexptparams) 
  ...
  % set nomogram parameters and run mrmrnomogram function or script
  param4par.mrmr.alpha = prelexptparam.alpha;
  param4par.mrmr.hasintercept = true; % false for finding and true for the final nomogram
  param4par.mrmr.clnoption = 'lasso|mse'; % 'lasso|mse' 
  param4par.seed = 20240620; % 20240620
  
  [trnresult,valresult,lclsubset,newsubset,param4par] = mrmrnomogram(CAND,CLN,li,param4par,prelexptparam);
...
%% final FS and training, save mrmr.mat
clc;
%%% run section 2 with the optimal parameter combination (change function mrmrnomogram to a script file) 
...
%% independent test
clear; clc; load('mrmr.mat'); warning('off');
clear;clc;
run('loadds.m');
