function [loss,YFIT] = ccrossval(funorStr,varargin)
%CCROSSVAL customize cross-validation for classo.
%   VALS = CCROSSVAL(FUN,X) performs 10-fold cross-validation for the
...
%
%   VALS = CCROSSVAL(FUN,X,Y,...) is used when there are multiple variables
...
%                  computing cross validation partitions. This argument can
%                  be created by a call to STATSET. CROSSVAL uses the
%                  following fields:
%                      'UseParallel'
%                      'UseSubstreams'
%                      'Streams'
...
%      cvMse = ccrossval('mse',x,y,'predfun',regf)
%
...
%      cvMCR = ccrossval('mcr',meas,y,'predfun', classf,'partition',cp)
%
...
%          'order',yorder);
%      cfMat = ccrossval(f,meas,species,'partition',cp);
...
pnames = { 'kfold'  'holdout'  'leaveout' 'mcreps' 'stratify' ...
  'partition' 'predfun' 'options' };
dflts =  { []        []          []        1          []         []...
  []        statset('crossval') };
[nfolds,holdout, leaveout, mcreps, stratify, cvp, predfun, ...
  parallelOption] = internal.stats.parseArgs(pnames, dflts, varargin{:});
...
if strncmp(firstInputType,'lossMeasure',3)
    ...
    if ~ismse %mcr
        ...
        elseif (iscellstr(data{end}) || isnumeric(data{end}) || ...
                islogical(data{end}) || ...
                isa(data{end},'categorical') ) && isvector(data{end})
            data{end}=nominal(data{end});
        else
...
if strcmp(firstInputType,'func') % the first input is a function handle
    ...
    [funResult,yfit] = getFuncVal(1, nData, cvp, data, funorStr, []);
    szFunResult = size(funResult);
    loss(numiter,:) = funResult(:)';
    loss(1,:) = funResult(:)';
    YFIT = zeros(size(data{1},1),size(yfit,2));
    YFIT(cvp.test(1),:) = yfit;
    % train = cvp.training(i);
    % test = cvp.test(i);
    
    if useParallel
        yfits = cell(numiter,1);
        parfor j=2:numiter
            mcrep = 1 + floor((j-1)/numtests);
            i = 1 + mod(j-1,numtests);  % i-th fold within this mc rep
            cvpj = cvparray{mcrep};
            [funResult,yfit] = getFuncVal(i, nData, cvpj, data, funorStr, szFunResult);
        ...
        for mr = 1:mcreps
            ...
            for i = loopstart:cvp.NumTestSets
                [funResult,yfit] = getFuncVal(i, nData, cvp, data, funorStr, szFunResult);
                loss(offset+i,:) = funResult(:)';
                YFIT(cvp.test(offset+i),:) = yfit;
            end
        end  % for mr=1:mcreps
    end
    
else % the first input is the loss measure
    error(['Not supported now for loss measure in this custom crossval ' ...
      '(i.e., mse or mcr are not supported for funorStr)']);
...
function [funResult,yfit] = evalFun(fun,arg)
try
    [funResult,yfit] = fun(arg{:});
catch ME
...
function [funResult,yfit] = getFuncVal(i, nData, cvp, data, funorStr, szFunResult)
arg = cell(2*nData,1);
train = cvp.training(i);
test = cvp.test(i);
...
[funResult,yfit] = evalFun(funorStr,arg(:));
