...
if isequal(unique(Y_train),[0;1])
  Y_train(Y_train==0) = -1;
elseif isequal(unique(Y_train),[1;2])
  Y_train(Y_train==2) = -1;
elseif isequal(unique(Y_train),[-1;+1])
  
else
  error('Not supported labels. Please relabel or update muteinf function.');
end
