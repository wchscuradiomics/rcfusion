...
nlims  = [fix(length(li.trnlabels)/10)]';% for radiomics_fs and nomogram_fs
nomfsmethods = {'lasso','rfe','univaranalysis|0.1','univaranalysis|0.01','univaranalysis|0.05'}; % for nomogram_fs
prelexptparams = ctable2struct(meshvarargin2grid(csvs,difsigtstthresholds,corrcoefthresholds,nlims,... % for radiomics_fs
  alphas,nomfsmethods)); % for nomogram_fs

xlsxname = 'rfe_prelexptresults.xlsx';
...
parfor i=1:height(prelexptparams) 
  ...
  % set nomogram parameters and run rfenomogram function or script
  param4par.rfe.alpha = prelexptparam.alpha;
  param4par.rfe.nlim = prelexptparam.nlim;
  param4par.rfe.hasintercept = true; %%% false for finding and true for the final nomogram
  param4par.rfe.clnoption = 'lasso|mse'; % 'lasso|mse' 
  param4par.seed = 20240620; % 20240620
  
  [trnresult,valresult,lclsubset,newsubset,param4par] = rfenomogram(CAND,CLN,li,param4par,prelexptparam);
  param = param4par; run('rfenomogram.m'); % for final experiment
...
%% final FS and training, save rfe.mat
clc;
%%% run section 2 with the optimal parameter combination (change function rfenomogram to a script file) 
...
%% independent test
clear; clc; load('rfe.mat'); warning('off');
clear;clc;
run('loadds.m');
