function [x,util] = ablxmutnomogram(CAND,CLN,li,param) % used for RPP (ablation experiment)

% subset based on ga
rng(param.seed);
[x,util] = ablxmutga(CAND(li.trnindices,:),CLN(li.trnindices,:),li.trnlabels,li.cvcvp,param); 
isxdisplayed = false;
for iindividual = 1:height(util.X)
  ...
  [lclsubset,B,fitinfo,bstlindex] = fs4lcl(LCL(li.trnindices,:),li.trnlabels,li.cvcvp,param.ablxmut.alpha);
  ...
  NEW = [radscores CLN];
  [newsubset,B2,fitinfo2] = fs4new(NEW(li.trnindices,:),li.trnlabels,li.cvcvp,param.ablxmut.alpha,param.ablxmut.clnoption);  
  TRN = NEW(li.trnindices,newsubset); 
  [nom,cvscores] = lrm(TRN,li.trnlabels,param.ablxmut.hasintercept,li.cvcvp,true);
  [mcvauc,aucs] = meancvaucs(li.cvcvp,li.trnlabels,cvscores);
  nomcoefs = nom.Coefficients.Estimate; if param.ablxmut.hasintercept, nomcoefs=nomcoefs(2:end); end
