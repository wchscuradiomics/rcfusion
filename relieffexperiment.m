...
nomfsmethods = {'relieff','lasso','univaranalysis|0.1','univaranalysis|0.01','univaranalysis|0.05'};
...
prelexptparams = ctable2struct(meshvarargin2grid(csvs,difsigtstthresholds,corrcoefthresholds,alphas,...
  nlims,ks,nomfsmethods)); % criterias

xlsxname = 'relieff_prelexptresults.xlsx';
...
parfor i=1:height(prelexptparams)
  ...
  % set nomogram parameters and run relieffnomogram function or script
  param4par.relieff.alpha = prelexptparam.alpha;
  param4par.relieff.hasintercept = true; % false for finding and true for the final nomogram
  param4par.relieff.clnoption = 'lasso|mse'; % 'lasso|mse' 
  param4par.seed = 20240620; % 20240620
  
  [trnresult,valresult,lclsubset,newsubset,param4par] = relieffnomogram(CAND,CLN,li,param4par,prelexptparam);
...
%% final FS and training, save relieff.mat
clc;
%%% run section 2 with the optimal parameter combination (change function relieffnomogram to a script file) 
...
%% independent test
clear; clc; load('relieff.mat'); warning('off');
clear;clc;
run('loadds.m');
