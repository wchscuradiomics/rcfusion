function [trnresult,valresult,lclsubset,newsubset,param] = ilnomogram(CAND,CLN,li,param,prelexptparam)

% select radiomic features based on il and build a radscore
% rng(param.seed);
[radfeatsindices,radfeatscores] = ILFS(CAND(li.trnindices,:),li.trnlabels,prelexptparam.tt); 
...
if strcmpi(prelexptparam.nomfsmethod,'lasso')
  [newsubset,B2,fitinfo2] = fs4new(NEW(li.trnindices,:),li.trnlabels,li.cvcvp,param.il.alpha,param.il.clnoption);
elseif contains(prelexptparam.nomfsmethod,'univaranalysis','IgnoreCase',true)
  ...
  param.nom.tt = prelexptparam.tt;
  [newsubset,nomfeatsindices,~] = fsil(NEW(li.trnindices,:),li.trnlabels,param.nom.tt,'none|nan',...
...
[nom,nomcvscores] = lrm(TRN,li.trnlabels,param.il.hasintercept,li.cvcvp,true);
[nommcvauc,nomkcvaucs] = meancvaucs(li.cvcvp,li.trnlabels,nomcvscores);
nomcoefs = nom.Coefficients.Estimate; if param.il.hasintercept, nomcoefs=nomcoefs(2:end); end
