    ...
    for idifsigtstthreshold = 1:length(difsigtstthresholds)
      for icorrcoefthreshold = 1:length(corrcoefthresholds)
        for itime = 1:length(times)
          ...
          param.our.hasintercept = true; % false for finding and true for the final nomogram
          param.our.szlocal = round( times(itime) * width(CAND) ); % times(itime) * width(CAND)
          param.our.clnoption = 'lasso|mse'; % 'lasso|mse'
          param.our.radconrithreshold = 0.61203; % 0.61202 0.61203 0.61204 0.61205 0.61206 0.612067(better) 0.612068
          param.our.fitcoefs = [0.90 0.08 0.02]; % [0.75 0.249 0.001] 0.7 0.135 0.165
...
%% final FS and training, save our.mat
clc;
%%% run section 2 with the optimal parameter combination (change function ournomogram to a script file) 
...
%% independent test
clear; clc; load('our.mat'); warning('off');
clear;clc;
run('loadds.m');
