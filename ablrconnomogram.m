function [x,util] = ablrconnomogram(CAND,CLN,li,param)

% subset based on ga
rng(param.seed);
[x,util] = ablrconga(CAND(li.trnindices,:),CLN(li.trnindices,:),li.trnlabels,li.cvcvp,param); 
isxdisplayed = false;
for iindividual = 1:height(util.X)
  ...
  [lclsubset,B,fitinfo,bstlindex] = fs4lcl(LCL(li.trnindices,:),li.trnlabels,li.cvcvp,param.ablrcon.alpha,...
    fix(param.ablrcon.idfmax*length(li.trnindices)/10));
  ...
  NEW = [radscores CLN];
  [newsubset,B2,fitinfo2] = fs4new(NEW(li.trnindices,:),li.trnlabels,li.cvcvp,param.ablrcon.alpha,param.ablrcon.clnoption);  
  TRN = NEW(li.trnindices,newsubset);  
  [nom,cvscores] = lrm(TRN,li.trnlabels,param.ablrcon.hasintercept,li.cvcvp,true);
  [mcvauc,aucs] = meancvaucs(li.cvcvp,li.trnlabels,cvscores);
  nomcoefs = nom.Coefficients.Estimate; if param.ablrcon.hasintercept, nomcoefs=nomcoefs(2:end); end
