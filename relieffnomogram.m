function [trnresult,valresult,lclsubset,newsubset,param] = relieffnomogram(CAND,CLN,li,param,prelexptparam)

% select radiomic features based on relieff and build a radscore
% rng(param.seed);
[radfeatsindices,radfeatscores] = relieff(CAND(li.trnindices,:),li.trnlabels,prelexptparam.k,'method','classification'); 
...
if strcmpi(prelexptparam.nomfsmethod,'lasso')
  [newsubset,B2,fitinfo2] = fs4new(NEW(li.trnindices,:),li.trnlabels,li.cvcvp,param.relieff.alpha,param.relieff.clnoption);
elseif contains(prelexptparam.nomfsmethod,'univaranalysis','IgnoreCase',true)
  ...
  param.nom.k = prelexptparam.k;
  [newsubset,nomfeatsindices,~] = fsrelieff(NEW(li.trnindices,:),li.trnlabels,param.nom.k,'classification',...
    'drop',width(NEW),false);
...
[nom,nomcvscores] = lrm(TRN,li.trnlabels,param.relieff.hasintercept,li.cvcvp,true);
[nommcvauc,nomkcvaucs] = meancvaucs(li.cvcvp,li.trnlabels,nomcvscores);
nomcoefs = nom.Coefficients.Estimate; if param.relieff.hasintercept, nomcoefs=nomcoefs(2:end); end
